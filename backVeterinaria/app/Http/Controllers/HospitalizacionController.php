...
class HospitalizacionController extends Controller
{
    public function getHospitalizacionesActivas()
    {
        $busqueda = DB::table('hospitalizaciones as h')
            ...
            ->select('h.id', 'h.created_at', 'h.numero_camilla', 'm.nombre as nombreMascota', 'cl.nombre as nombreDuenio', 'tm.descripcion as tipoMascota')
            ->where('h.id_estado', '1')
    ...
    public function getHospitalizacionPorId($id)
    {
        $busqueda = DB::table('hospitalizaciones as h')
            ...
            ->select('h.id', 'h.created_at', 'h.motivo', 'h.id_estado', 'h.numero_camilla', 'm.nombre as nombreMascota', 'm.id as id_mascota', 'u.name as nombreVetHosp', 'm.fecha_nacimiento', 'cl.nombre as nombreDuenio', 'cl.rut', 'cl.numero', 'tm.descripcion as tipoMascota')
            ->where('h.id', $id)
            ->get();
        if (!$busqueda->isEmpty()) {
